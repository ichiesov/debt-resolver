...
from app.domain.models import ExpenseEntry

from .base import BaseRepository

_TABLE = "expense_entries"


class ExpenseRepository(BaseRepository):
    def _row_to_model(self, row: dict[str, Any]) -> ExpenseEntry:
        return ExpenseEntry(
            id=uuid.UUID(row["id"]),
    ...
    async def get_all_active(self) -> list[ExpenseEntry]:
        response = (
    ...
    async def get_by_date_range(self, from_date: date, to_date: date) -> list[ExpenseEntry]:
        one_time_resp = (
            await self._client.table(_TABLE)
            .select("*")
            .eq("user_id", self._user_id)
